/*
 * Adapted from Everything Claude Code (ECC) scripts/lib/session-manager.js
 ...
 * Provides core session CRUD operations for listing, loading, and managing
 * session-data markdown summaries.
 *
 * Sessions are stored as markdown files under <agent-data-home>/session-data/
 * (see resolveAgentDataHome from ./agent-data-home), with legacy read
 * compatibility for older session directories:
 * - YYYY-MM-DD-session.tmp (old format)
 * - YYYY-MM-DD-<short-id>-session.tmp (new format)
 *
 * Generated summary blocks are wrapped in PAIRED markers:
 *   <!-- ESCC:SUMMARY:START -->
 *   ...
 *   <!-- ESCC:SUMMARY:END -->
 * A migration regex tolerates the legacy single ECC marker so older files
 * are upgraded in place on the next write.
 */
...
// Paired summary markers. The generated summary block lives between these.
const SUMMARY_START_MARKER = '<!-- ESCC:SUMMARY:START -->';
const SUMMARY_END_MARKER = '<!-- ESCC:SUMMARY:END -->';

// Legacy single marker emitted by older ECC/ESCC builds before paired markers
// existed. Matched so a single legacy block can be migrated to paired markers.
const LEGACY_SUMMARY_MARKER = '<!-- ECC:SUMMARY -->';
...
// "2026-02-01-frontend-1-session.tmp", and "2026-02-01-ChezMoi_2-session.tmp"
...
/**
 * Build a paired-marker summary block from a summary body string.
 * @param {string} summaryBody - Inner markdown for the summary block
 * @returns {string} Block wrapped in paired START/END markers
 */
function buildSummaryBlock(summaryBody) {
  const body = (summaryBody == null ? '' : String(summaryBody)).trim();
  return `${SUMMARY_START_MARKER}\n${body}\n${SUMMARY_END_MARKER}`;
}

/**
 * Extract the inner body of the summary block from session content.
 * Tolerant of the legacy single ECC marker.
 * @param {string} content - Session markdown content
 * @returns {string|null} Inner summary body, or null if no block present
 */
function extractSummaryBlock(content) {
  ...
  const pairedMatch = content.match(pairedRegex);
  if (pairedMatch) {
    return pairedMatch[1].trim();
  }

  // Migration: legacy single marker. Treat everything from the legacy marker
  // up to the next heading section as the summary body.
  const legacyRegex = new RegExp(
    `${escapeRegExp(LEGACY_SUMMARY_MARKER)}\\s*\\n([\\s\\S]*?)(?=\\n###\\s|$)`
  );
...
/**
 * Insert or update the paired summary block inside session content.
 * - If paired markers exist, the block between them is replaced.
 * - If only the legacy single marker exists, it is migrated to paired markers.
 * - Otherwise the new block is prepended.
 *
 * Function replacers are used so summary text containing `$&`, `$1`, etc. is
 * treated literally and cannot corrupt the persisted summary.
 *
 * @param {string} content - Existing session markdown content
 * @param {string} summaryBody - New summary body
 * @returns {string} Updated content
 */
function upsertSummaryBlock(content, summaryBody) {
  const block = buildSummaryBlock(summaryBody);
  const existing = content && typeof content === 'string' ? content : '';

  if (existing.includes(SUMMARY_START_MARKER) && existing.includes(SUMMARY_END_MARKER)) {
    const pairedRegex = new RegExp(
      `${escapeRegExp(SUMMARY_START_MARKER)}[\\s\\S]*?${escapeRegExp(SUMMARY_END_MARKER)}`
    );
    return existing.replace(pairedRegex, () => block);
  }

  if (existing.includes(LEGACY_SUMMARY_MARKER)) {
    // Migration path: replace the legacy marker (and its trailing body, up to
    // the next heading) with a fully paired block.
    const legacyRegex = new RegExp(
      `${escapeRegExp(LEGACY_SUMMARY_MARKER)}[\\s\\S]*?(?=\\n###\\s|$)`
    );
    return existing.replace(legacyRegex, () => block);
  }
...
function parseSessionFilename(filename) {
  ...
  return {
    ...
    shortId,
    date: dateStr,
    // Use local-time constructor (consistent with validation above).
...
/**
 * Get the full path to a session file under the active session-data dir
...
function parseSessionMetadata(content) {
  ...
  // Extract the generated summary block (paired markers, legacy-tolerant)
  metadata.summary = extractSummaryBlock(content);
...
/**
 * Get the most recent session modified within the last N days.
 * @param {number} days - Lookback window in days (default 7)
 * @param {boolean} includeContent - Include parsed content/metadata/stats
 * @returns {object|null} Most recent matching session record, or null
 */
function getLatestSessionWithinDays(days = 7, includeContent = false) {
  const daysNum = Number(days);
  const safeDays = Number.isNaN(daysNum) ? 7 : Math.max(0, daysNum);
  const cutoff = Date.now() - safeDays * MILLIS_PER_DAY;

  const sessions = getSessionCandidates();
  const recent = sessions.find(session => {
    const modified = session.modifiedTime instanceof Date
      ? session.modifiedTime.getTime()
 ...
 * @returns {Array<object>} Session records (without content)
 */
function listRecentSessions(limit = 10) {
  const { sessions } = getAllSessions({ limit, offset: 0 });
  return sessions;
}
...
/**
 * Write or update only the generated summary block within a session file,
 * preserving the rest of the content. Inserts paired markers (and migrates
 * legacy single markers). Creates the file with just the block if absent.
 * @param {string} sessionPath - Full path to session file
 * @param {string} summaryBody - New summary body (inner markdown)
 * @returns {boolean} Success status
 */
function writeSessionSummary(sessionPath, summaryBody) {
  const existing = getSessionContent(sessionPath) || '';
  const updated = upsertSummaryBlock(existing, summaryBody);
  return writeSessionContent(sessionPath, updated);
}
...
/**
 * Resolve the active session-data directory (under agent data home).
 * @returns {string} Absolute path to the session-data directory
 */
