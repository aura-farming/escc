...
/*
 * Adapted from Everything Claude Code (ECC) scripts/lib/session-bridge.js
 * (MIT, (c) Affaan Mustafa) https://github.com/affaan-m/ECC.
 * Re-namespaced ECC_*->ESCC_*.
 */

/**
 * Shared session bridge utilities for ESCC hooks.
 *
 * The bridge file is a small JSON aggregate in the OS temp dir that
 * allows statusline, metrics-bridge, and context-monitor to share state
 * without scanning large JSONL logs on every invocation. This bridge is
 * the SINGLE source of metrics — there is no separate per-id cost file.
 */
...
function getBridgePath(sessionId) {
  return path.join(os.tmpdir(), `escc-metrics-${sessionId}.json`);
}
 ...
 * writers (e.g. PostToolUse `escc-metrics-bridge` and the background
 * `escc-statusline`, both writing to the same session bridge) do not
 ...
 * `scripts/hooks/escc-metrics-bridge.js` for the cost-warning cache;
 * this primitive applies it to the session-bridge too.
 *
 * @param {string} sessionId - Already-sanitized session ID
 * @param {object} data
 */
...
function resolveSessionId() {
  const raw = process.env.ESCC_SESSION_ID || process.env.CLAUDE_SESSION_ID || '';
