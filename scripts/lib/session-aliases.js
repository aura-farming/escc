/*
 * Adapted from Everything Claude Code (ECC) scripts/lib/session-aliases.js
 ...
 * Manages session aliases stored in $ESCC_AGENT_DATA_HOME/session-aliases.json
 * (default ~/.claude).
 */
...
// Aliases file path (under the resolved agent-data-home).
function getAliasesPath() {
  return path.join(resolveAgentDataHome(), 'session-aliases.json');
}
