/*
 * Adapted from Everything Claude Code (ECC) scripts/lib/install-state.js
 * (MIT, (c) Affaan Mustafa) https://github.com/affaan-m/ECC.
 * Re-namespaced ECC_*->ESCC_*.
 *
 * Create, read, write, and validate the ESCC install-state record. Uses ajv
 * when available and falls back to a hand-rolled validator that stays
 * byte-aligned with schemas/install-state.schema.json for bare checkouts.
 */
...
function createFallbackValidator() {
  const validate = state => {
    ...
    if (state.schemaVersion !== 'escc.install.v1') {
      pushError('/schemaVersion', 'must equal escc.install.v1');
    }
