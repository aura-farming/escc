...
test('workspace is keyed by rep identity, not git remote (I1)', () => {
  ...
  const dirA = withEnv({ ESCC_INSTINCT_HOME: home, ESCC_REP_IDENTITY: 'rep-a@company.example' }, () => store.resolveWorkspaceDir());
  const dirB = withEnv({ ESCC_INSTINCT_HOME: home, ESCC_REP_IDENTITY: 'rep-b@company.example' }, () => store.resolveWorkspaceDir());
