...
test('tracks accounts touched from an email domain', () => {
  const home = freshHome();
  withEnv({ ESCC_AGENT_DATA_HOME: home }, () => {
    hook.run(postInput('mcp__claude_ai_Gmail__create_draft', { to: 'jane@acme.io', body: 'hi' }));
    const rec = readRecord(home);
    assert.ok(rec.accounts.includes('domain:acme.io'));
  });
});
