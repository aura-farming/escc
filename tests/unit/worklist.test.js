...
test('buildReviewPack splits a worklist into sendable vs excluded with reasons', () => {
  const items = [
    { id: 'good', draft: { to: 'a@b.example', subject: 'Hi', body: 'You could cut manual work across your teams — worth a quick look?' }, records: { notes: [], open_deals: [], priorEngagement: false } },
    { id: 'open-deal', draft: { to: 'c@d.example', subject: 'Hi', body: 'You could save hours — keen?' }, records: { open_deals: [{ id: '1' }] } },
    { id: 'wiifm', draft: { to: 'e@f.example', subject: 'Hi', body: "Here's a Example Co vs competitor-x comparison." }, records: { priorEngagement: false } },
  ];
...
test('buildReviewPack never throws on a malformed item', () => {
  const pack = buildReviewPack([null, { id: 'ok', draft: { to: 'a@b.example', subject: 'Hi', body: 'You could save hours.' }, records: {} }], { now: '2026-06-23' });
