...
test('E2E: `escc dnc record` makes the send-gate BLOCK an already-approved draft', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    const toolInput = { to: 'sam@company.example', subject: 'Thursday', body: 'Hi Sam, quick one.' };
    ...
    const res = escc.run(['dnc', 'record', '--key', 'sam@company.example', '--source', 'email', '--reason', 'opt-out: "please stop"']);
...
test('record --scope account canonicalizes a bare domain and suppresses every contact there', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    const res = escc.run(['dnc', 'record', '--key', 'company.example', '--scope', 'account', '--reason', 'org-wide opt-out']);
    assert.equal(res.code, 0, res.text);
    assert.equal(res.data.key, identity.accountKey('company.example'), 'stored under the ADR-0018 canonical account key');

    const toolInput = { to: 'someone.new@company.example', subject: 'Hi', body: 'Hello there.' };
    approveDraft(toolInput);
    const blocked = gate.run(gateInput(toolInput));
...
test('record refuses a contact-scope key that is not an email (would gate nothing)', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    const res = escc.run(['dnc', 'record', '--key', 'company.example']);
...
test('clear refuses without --evidence; with evidence it lifts the block and the gate passes again', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    const toolInput = { to: 'sam@company.example', subject: 'Re-engage', body: 'Hi Sam.' };
    escc.run(['dnc', 'record', '--key', 'sam@company.example', '--reason', 'asked us to stop']);

    const refused = escc.run(['dnc', 'clear', '--key', 'sam@company.example']);
    assert.equal(refused.code, 1);
    assert.match(refused.text, /--evidence/);
    assert.ok(dnc.findActiveBlock({ key: 'sam@company.example' }), 'the block must survive a refused clear');

    const cleared = escc.run(['dnc', 'clear', '--key', 'sam@company.example', '--evidence', 'signed re-consent form 2026-07-15']);
    assert.equal(cleared.code, 0, cleared.text);
    assert.match(cleared.data.reason, /re-consent: signed re-consent form/);
    assert.equal(dnc.findActiveBlock({ key: 'sam@company.example' }), null);
...
test('check mirrors the gate: reports a contact block, and an account block for a mere email', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    assert.match(escc.run(['dnc', 'check', '--key', 'sam@company.example']).text, /Not blocked/);

    escc.run(['dnc', 'record', '--key', 'company.example', '--scope', 'account', '--reason', 'org-wide']);
    const res = escc.run(['dnc', 'check', '--key', 'sam@company.example']);
    assert.match(res.text, /BLOCKED \(account-scope\)/);

    const asJson = escc.run(['dnc', 'check', '--key', 'sam@company.example', '--json']);
...
test('list shows folded rows (and --json round-trips); unknown actions are refused', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    escc.run(['dnc', 'record', '--key', 'sam@company.example', '--reason', 'asked us to stop']);
    assert.match(escc.run(['dnc', 'list']).text, /sam@company\.example — blocked indefinitely/);
...
test('a display-name recipient ("Sam <sam@…>") cannot slip past the blocklist', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    const toolInput = { to: 'Sam Jones <sam@company.example>', subject: 'Hi', body: 'Quick one.' };
    ...
    assert.match(blocked.stderr, /sam@company\.example is on the do-not-contact/i);
  });
});

test('a multi-recipient send is blocked when ANY addressee is suppressed', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    const toolInput = { to: 'ok@corp.example, Sam <sam@company.example>', subject: 'Team', body: 'Hello both.' };
    approveDraft(toolInput);
...
test('an account-scope block also catches a display-name recipient at that domain', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    escc.run(['dnc', 'record', '--key', 'company.example', '--scope', 'account', '--reason', 'org-wide']);
    const toolInput = { to: 'New Person <new.person@company.example>', subject: 'Hi', body: 'Hello.' };
    approveDraft(toolInput);
    const blocked = gate.run(gateInput(toolInput));
    assert.ok(blocked && blocked.exitCode === 2);
    assert.match(blocked.stderr, /account is on the do-not-contact/i);
...
test('record refuses an unparseable --not-before instead of writing a block that never fires', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    const res = escc.run(['dnc', 'record', '--key', 'sam@company.example', '--not-before', 'next tuesday']);
    assert.equal(res.code, 1);
...
test('FAIL CLOSED: a stored row with a garbled not_before still blocks (never reads as expired)', () => {
  withEnv({ ESCC_AGENT_DATA_HOME: freshStateHome() }, () => {
    dnc.recordDoNotContact({ key: 'sam@company.example', notBefore: 'corrupted-date-value', reason: 'timing block' });
    assert.ok(dnc.findActiveBlock({ key: 'sam@company.example' }), 'garbage not_before must read as still-blocked');
  });
});
