...
| `prompt:capture-correction` | `*` | `capture-correction.js` | standard, strict | Learning-loop capture (v1.8.0): detects an explicit rep CORRECTION in the prompt (`config/correction-patterns.json`, conservative) and appends a `user_correction` observation — the instinct engine's strongest signal, which previously had no writer. Nothing auto-applies: drafts still pass the I7 human review gate (`/instinct-status`). Skips slash commands and very long prompts (likely pasted third-party content). Silent observer; never blocks. |
...
| `post:outcome-capture` | `mcp__hubspot__manage_crm_objects` \| Calendar `create_event` | `outcome-capture.js` | all | Learning-loop keystone (v1.8.0): a successful HubSpot deal-STAGE write becomes a `deal_stage_advanced` / `closed_won` / `closed_lost` outcome and a Calendar event a `meeting_booked` outcome in the state-store ledger — which SessionEnd distillation reads to move instinct confidence on REAL results (I2). Payloads are whitelisted structured fields only (never free text — ADR-0012 discipline). Skips errored/truncated calls. Silent observer; never blocks. |
